...
def create_kafka_topic(topic):
    ...
    if "signalling." in topic:
        ...
        subprocess.call(['/bin/bash', '/opt/kafka/bin/kafka-topics.sh', '--create', '--zookeeper', zookeeper_ip_address+":2181", '--topic', topic, '--replica-assignment', '2:1'])
    elif ".italy_" in topic:
        subprocess.call(['/bin/bash', '/opt/kafka/bin/kafka-topics.sh', '--create', '--zookeeper', zookeeper_ip_address+":2181", '--topic', topic, '--replica-assignment', '3:1'])
    elif ".france_" in topic:
        subprocess.call(['/bin/bash', '/opt/kafka/bin/kafka-topics.sh', '--create', '--zookeeper', zookeeper_ip_address+":2181", '--topic', topic, '--replica-assignment', '4:1'])
    elif ".greece_" in topic:
        subprocess.call(['/bin/bash', '/opt/kafka/bin/kafka-topics.sh', '--create', '--zookeeper', zookeeper_ip_address+":2181", '--topic', topic, '--replica-assignment', '5:1'])
    else:
